...
@section('title', $title . ' — Process Flow JBIS')
...
@push('head')
    ...
    <style>
        ...
        .step-card { transition: all 0.1s ease; }
        @media print {
...
@section('content')
<div class="max-w-4xl mx-auto bg-white rounded-2xl border border-gray-100 overflow-hidden">
    <div class="px-6 py-5 md:px-8">
        <div class="flex flex-wrap justify-between items-start gap-4 border-b border-gray-200 pb-4 mb-5">
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight text-gray-900 mt-2">{{ $title }}</h1>
            </div>
        ...
        @foreach ($sections as $section)
            <div class="mb-8 page-break-inside-avoid">
                <div class="flex items-start gap-3 mb-4">
                    <div class="w-1.5 h-8 rounded-full" style="background-color: {{ $section['color'] }};"></div>
                    <div class="flex-1">
                ...
                <div class="space-y-3 ml-2">
                    @foreach ($section['steps'] as $step)
                        <div class="step-card bg-white border border-gray-200 rounded-xl p-4">
        ...
        <div class="mt-6 pt-3 border-t border-gray-200 flex justify-between items-center text-xs text-gray-400">
            <span>{{ $stepsCount }} {{ $locale === 'fr' ? 'étape(s)' : 'step(s)' }}</span>
            <span class="font-mono">Process Flow JBIS · {{ $generatedAt }}</span>
        </div>
    </div>
</div>
@endsection
